    ...
    if (deferredPrompt) {
      if (isIos() && !isInStandaloneMode()) {
        if ('standalone' in window.navigator && window.navigator['standalone']) {
          ...
          alert('Toque no botão de compartilhamento e selecione "Adicionar à Tela Inicial" para instalar o aplicativo.');
        }
      } else {
        deferredPrompt.prompt();
        const { outcome } = await deferredPrompt.userChoice.then((choiceResult: InstallPromptResult) => {
  ...
  return (
    <div className={`${styles.container} ${styles.cardContainer}`}>
      <div className={styles.cardInfo}>
        ...
        <p>
          <strong>Titulo:</strong> {siteConfig.data.sections.configurations?.title}
        </p>
        <p>
        ...
        {setupButtonVisible && !navigator.userAgent.includes('iPhone') && (
          <button onClick={installApp}>Baixar o App</button>
        )}
        {navigator.userAgent.includes('iPhone') && (
          <button onClick={addToHomeScreen}>Como Baixar o App (iOS)</button>
        )}
      </div>
    </div>
  );
